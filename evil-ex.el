...
(defvar evil--ex-shortcut-expression nil
  "Ex evaluation tree to use instead.")
...
`evil-ex-initial-input', if non-nil, is appended to the line."
  ...
  (let ((buffer (current-buffer))
        (previous-command (when evil-want-empty-ex-last-command
                            (car evil-ex-history)))
        s evil--ex-cmd evil--ex-argument-handler evil--ex-shortcut-expression)
    ...
    (if evil--ex-shortcut-expression
        (eval evil--ex-shortcut-expression t)
...
in case of incomplete or unknown commands."
  (when (and beg (eq this-command #'self-insert-command))
    (let ((cmd (lookup-key evil-ex-shortcut-map (minibuffer-contents-no-properties))))
      (when (commandp cmd)
        (setq evil--ex-shortcut-expression `(call-interactively #',cmd))
        (exit-minibuffer))))
